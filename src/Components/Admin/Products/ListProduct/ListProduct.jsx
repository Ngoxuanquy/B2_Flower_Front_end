  ...
  const handleOpenUpdate = async (confirm) => {
    ...
    if (confirm && selectedProductId) {
      ...
      if (result) {
        ...
        setSnackbarMessage("Lỗi khi cập nhật sản phẩm!");
        setSnackbarSeverity("error");
      }
                    ...
                    <td>
                      <div className={cx("actions")}>
                        {(roles.includes("EDIT") || roles.includes("ADMIN")) && (
                          <Button className={cx("success")} color="success" onClick={() => handleClickOpenUpdate(item._id)}>
                            <FaPencil />
                          </Button>
                        )}
                        {(roles.includes("DELETE") || roles.includes("ADMIN")) &&
                          (item.product_quantity === 0 ? (
                            <Button className={cx("error")} color="error" onClick={() => handleClickOpen(item._id)}>
                              <MdOutlineBlock />
                            </Button>
                          ) : (
                            <div>
