...
import { FaBoltLightning, FaCircleUser } from "react-icons/fa6";
...
const Sidebar = () => {
  ...
  return (
    <div className={`sidebar ${isToggled === true ? "toggled" : ""}`}>
      ...
      {(roles.includes("DESTROYFLOWERS") || roles.includes("ADMIN")) && (
        <SideBarItem
          ...
          subItems={
            roles.includes("ADMIN")
              ? ["Danh sách hủy hoa", "Danh sách đã duyệt", "Tạo báo cáo"]
              : null
          }
      ...
      <div className="logoutWrapper">
        {isToggled ? (
          <IconButton onClick={handleLogout} className="logoutIcon">
            <TbLogout className="logoutIconSVG" />
          </IconButton>
        ) : (
