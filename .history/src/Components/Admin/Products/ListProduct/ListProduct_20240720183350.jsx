...
const ListProduct = ({ apis, fetchProducts }) => {
  ...
  useEffect(() => {
    if (apis && Array.isArray(apis)) {
      ...
      setLoading(true); // Nếu không có dữ liệu, vẫn giữ trạng thái loading
    }
  }, [apis]);
  useEffect(() => {
        ...
        <div className="col-md-3 " style={{ marginLeft: "10px" }}>
          <FormControl sx={{ m: 1, minWidth: 120, width: "100%" }} size="small">
            ...
            >
              ...
              <MenuItem value={10}>Hoa</MenuItem>
              <MenuItem value={20}>hộp quà</MenuItem>
              <MenuItem value={30}>Gấu bông</MenuItem>
            </Select>
          </FormControl>
        </div>
      </div>
              ...
              {productsToDisplay?.map((item, index) => (
                <React.Fragment key={item._id}>
                  <tr>
                    ...
                    <td styled={{ display: "block" }}>
                      {item.product_discount > 0 && (
                        <del className={cx("old")}>{item.product_price}</del>
                      )}
