...
const DashBoard = () => {
  ...
  const [selectedRange, setSelectedRange] = useState("lastMonth"); // Khởi tạo selectedRange
  ...
  const getApiTransactionOrder = () => {
    ...
    Promise.all([
      ...
      .then(([doneOrdersResponse, fullOrdersResponse]) => {
        ...
        calculateTotalPrice(doneOrdersResponse.metadata, selectedRange); // Pass the selected range
  ...
  const calculateTotalPrice = (orders, range) => {
    let total = 0;
    const now = new Date();

    orders.forEach((order) => {
      const orderDate = new Date(order.createdAt); // Giả sử mỗi đơn hàng có một trường `createdAt`
      let isInRange = false;

      switch (range) {
        case "lastDay":
          isInRange = orderDate > now.setDate(now.getDate() - 1);
          break;
        case "lastWeek":
          isInRange = orderDate > now.setDate(now.getDate() - 7);
          break;
        case "lastMonth":
          isInRange = orderDate > now.setMonth(now.getMonth() - 1);
          break;
        case "lastYear":
          isInRange = orderDate > now.setFullYear(now.getFullYear() - 1);
          break;
        default:
          isInRange = true;
          break;
      ...
      if (isInRange) {
        total += order.total_amounts; // Giả sử mỗi đơn hàng có một trường total_amounts
      }
    });
        ...
        <div className={cx("dashboardBoxWrapperRow")}>
          ...
          <div className="col-md-4" style={{ padding: 0, paddingLeft: "10px" }}>
            <div className={cx("boxnew", "graphBox")}>
              ...
              <p>
                {totalPrice?.toLocaleString()} vnđ trong {ranges[selectedRange]}
              </p>
              <div>
